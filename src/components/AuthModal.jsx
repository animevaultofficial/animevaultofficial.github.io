...
// hCaptcha's web challenge can fail inside the Android WebView/custom-scheme flow.
// Keep it on the public website, but don't block native authentication on an unavailable widget.
...
export default function AuthModal() {
  ...
  const handleGoogle = async () => {
    ...
    if (!result?.success) { setError(result?.message || 'Google sign-in could not be started.'); setLoading(false); return; }
    // Native OAuth intentionally stays in the system browser until the deep-link callback returns.
    if (!result.pending) setLoading(false);
  };
    ...
    <div className="auth-overlay" onClick={() => setShowAuthModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(5, 5, 10, 0.85)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'rgba(15, 15, 25, 0.7)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255, 26, 117, 0.2)', borderRadius: '16px', padding: '30px', width: '100%', maxWidth: '400px', position: 'relative' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={loading || (hcaptchaEnabled && !captchaToken)} style={{ width: '100%', padding: '12px', background: '#ff1a75', color: '#000', fontWeight: '900', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '0.85rem', transition: 'all 0.2s', marginBottom: '16px', boxShadow: '0 0 15px rgba(255,26,117,0.3)', opacity: loading || (hcaptchaEnabled && !captchaToken) ? 0.65 : 1 }}>{loading ? 'Processing...' : (authTab === 'login' ? (step === 'otp' ? 'Verify & Sign In' : 'Sign In') : 'Create Account')}</button>
          <div style={{ display: 'flex', alignItems: 'center', margin: '16px 0', gap: '12px' }}><div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.1)' }} /><span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 'bold' }}>OR</span><div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.1)' }} /></div>
          <button type="button" onClick={handleGoogle} disabled={loading} style={{ width: '100%', padding: '12px', background: '#ffffff', color: '#000', fontWeight: '900', border: 'none', borderRadius: '10px', cursor: loading ? 'wait' : 'pointer', fontSize: '0.85rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', transition: 'all 0.2s', opacity: loading ? 0.7 : 1 }}><svg viewBox="0 0 24 24" width="18" height="18" xmlns="http://www.w3.org/2000/svg"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>Continue with Google</button>
        </form>
      </div>
    </div>
  );
