...
const primaryNav = [
  ...
  ['/dramas-movies', 'Dramas & Movies', TvIcon],
  ...
  return <Suspense fallback={<RouteFallback />}><SubAccountGate><div className={`app-shell ${isTvMode ? 'tv-app-shell' : ''}`}>
    ...
    <header className="topbar">
      ...
      <nav className="topnav" aria-label="Primary navigation">
        {primaryNav.map(([to, label, Icon]) => <FocusableNavLink key={to} to={to} end={to === '/'} className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'}>{Icon && <Icon size={15} />}<span>{label}</span></FocusableNavLink>)}
        {user?.is_admin && <FocusableNavLink to="/admin/dashboard" className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'}><Award size={15} /><span>Admin</span></FocusableNavLink>}
      </nav>
    ...
    {isMobileMenuOpen && <>
      <div className="mobile-menu-overlay" role="presentation" onMouseDown={() => setIsMobileMenuOpen(false)} />
      <aside id="mobile-navigation" className="mobile-menu" aria-label="Mobile navigation" aria-hidden={!isMobileMenuOpen}>
        <div className="mobile-menu-header">
          <div className="mobile-menu-title"><img src={assetPath('logo.png')} alt="" aria-hidden="true" /><div><strong>AnimeVault</strong><span>Navigation</span></div></div>
          <button className="mobile-menu-close" type="button" aria-label="Close navigation menu" onClick={() => setIsMobileMenuOpen(false)}><X size={22} /></button>
        </div>
        <div className="mobile-menu-links">
          {primaryNav.map(([to, label, Icon]) => <FocusableNavLink key={to} to={to} end={to === '/'} onClick={() => setIsMobileMenuOpen(false)} className={({ isActive }) => isActive ? 'mobile-nav-link active' : 'mobile-nav-link'}>{Icon && <Icon size={18} />}<span>{label}</span></FocusableNavLink>)}
          <FocusableNavLink to="/about" onClick={() => setIsMobileMenuOpen(false)} className={({ isActive }) => isActive ? 'mobile-nav-link active' : 'mobile-nav-link'}><Info size={18} /><span>About</span></FocusableNavLink>
          {user?.is_admin && <FocusableNavLink to="/admin/dashboard" onClick={() => setIsMobileMenuOpen(false)} className={({ isActive }) => isActive ? 'mobile-nav-link active admin-link' : 'mobile-nav-link admin-link'}><Award size={18} /><span>Admin Dashboard</span></FocusableNavLink>}
        </div>
        <div className="mobile-menu-account">
          {user ? <FocusableLink to={ownProfilePath} onClick={() => setIsMobileMenuOpen(false)} className="mobile-account-link"><User size={18} /><span><strong>{user.username}</strong><small>View profile</small></span></FocusableLink> : !authLoading && <FocusableButton onClick={openLogin} className="mobile-account-link"><User size={18} /><span>Sign In</span></FocusableButton>}
          {user && <FocusableNavLink to="/settings" onClick={() => setIsMobileMenuOpen(false)} className="mobile-account-link"><Settings size={18} /><span>Settings</span></FocusableNavLink>}
        </div>
      </aside>
    </>}
    ...
    <style>{`
      .topbar-brand-wrap{display:flex;align-items:center;gap:.65rem;min-width:0;flex:0 0 auto}
      .topbar .brand{display:flex;align-items:center;gap:.65rem;min-width:0;white-space:nowrap}
      .topbar .brand img{height:34px;width:auto;display:block;flex:0 0 auto}
      .topbar .brand span{font-weight:900}
      .hamburger-btn{display:none;align-items:center;justify-content:center;background:transparent!important;border:1px solid transparent!important;color:#fff;cursor:pointer;padding:.45rem;border-radius:10px;flex:0 0 auto}
      .hamburger-btn:hover,.hamburger-btn:focus-visible{background:var(--white-05)!important;border-color:var(--white-10)!important}
      .header-profile{display:inline-flex;align-items:center;gap:.4rem;min-width:0;max-width:150px;white-space:nowrap;overflow:hidden}
      .header-profile span{overflow:hidden;text-overflow:ellipsis}
      .session-status{font-size:.78rem;color:var(--text-secondary);white-space:nowrap}
      .route-loading{min-height:35vh;display:grid;place-items:center;color:var(--text-secondary)}
      .loading-dot{width:9px;height:9px;border-radius:50%;background:var(--brand-color);box-shadow:0 0 18px rgba(255,26,117,.55)}
      .mobile-menu-overlay{position:fixed;inset:0;z-index:999;background:rgba(0,0,0,.68);backdrop-filter:blur(3px)}
      .mobile-menu{position:fixed;inset:0 auto 0 0;z-index:1000;width:min(86vw,350px);height:100dvh;display:flex;flex-direction:column;overflow:hidden;background:rgba(10,16,25,.985);border-right:1px solid var(--white-10);box-shadow:24px 0 70px rgba(0,0,0,.45);animation:mobileNavIn .18s ease-out}
      .mobile-menu-header{display:flex;align-items:center;justify-content:space-between;gap:1rem;padding:1rem 1rem .85rem;border-bottom:1px solid var(--white-10);flex:0 0 auto}
      .mobile-menu-title{display:flex;align-items:center;gap:.7rem;min-width:0}
      .mobile-menu-title img{width:34px;height:34px;object-fit:contain}
      .mobile-menu-title div{display:flex;flex-direction:column;min-width:0}
      .mobile-menu-title strong{font-size:1rem}
      .mobile-menu-title span{font-size:.72rem;color:var(--text-tertiary)}
      .mobile-menu-close{display:flex;align-items:center;justify-content:center;padding:.5rem;border-radius:9px;background:var(--white-05);border:1px solid var(--white-10);color:#fff;cursor:pointer}
      .mobile-menu-links{flex:1 1 auto;overflow-y:auto;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;padding:.7rem .65rem;display:flex;flex-direction:column;gap:.2rem;touch-action:pan-y}
      .mobile-nav-link{display:flex;align-items:center;gap:.8rem;padding:.75rem .8rem;border-radius:10px;color:var(--text-secondary);text-decoration:none;font-weight:650;min-height:44px}
      .mobile-nav-link:hover{background:var(--white-05);color:#fff}
      .mobile-nav-link.active{background:rgba(255,26,117,.13);color:var(--brand-color)}
      .mobile-nav-link.admin-link{margin-top:.45rem;border-top:1px solid var(--white-10);border-radius:0 0 10px 10px;padding-top:1rem}
      .mobile-menu-account{flex:0 0 auto;border-top:1px solid var(--white-10);padding:.65rem;display:flex;flex-direction:column;gap:.25rem;padding-bottom:calc(.65rem + env(safe-area-inset-bottom))}
      .mobile-account-link{display:flex!important;align-items:center;gap:.75rem;width:100%;padding:.75rem .8rem;border-radius:10px;color:var(--text-secondary);text-decoration:none;min-height:44px}
      .mobile-account-link:hover{background:var(--white-05);color:#fff}
      .mobile-account-link span{display:flex;flex-direction:column;min-width:0}
      .mobile-account-link small{color:var(--text-tertiary);font-size:.7rem}
      @keyframes mobileNavIn{from{transform:translateX(-18px);opacity:.7}to{transform:translateX(0);opacity:1}}
