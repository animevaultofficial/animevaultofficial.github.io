...
function RouteFallback() {
  return <div className="route-loading" role="status" aria-live="polite"><span className="loading-dot" /> Loading…</div>;
}

function App() {
  useReminderNotifications();
  const { user, authLoading, setShowAuthModal, setAuthTab } = useUser();
  const { activeSubAccount, subAccounts } = useUser();
  ...
  useEffect(() => {
    ...
    async function loadSettings() {
      try {
        ...
        if (!cancelled) console.warn('Global settings load skipped:', err?.message);
      }
    }
    loadSettings();
        ...
        <div className={`app-shell ${isTvMode ? 'tv-app-shell' : ''}`}>
          ...
          <header className="topbar">
            <div className="topbar-brand-wrap">
              <button className="hamburger-btn" aria-label="Open menu" aria-expanded={isMobileMenuOpen} onClick={() => setIsMobileMenuOpen(v => !v)}>
                {isMobileMenuOpen ? <X size={24} /> : <Menu size={24} />}
              </button>
              <FocusableLink to="/" className="brand"><img src={assetPath('logo.png')} alt="AnimeVault" /><span>AnimeVault</span></FocusableLink>
            </div>
            <nav className="topnav">
              ...
              {user?.is_admin && <FocusableNavLink to="/admin/dashboard" className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'}><Award size={16} /> Admin</FocusableNavLink>}
            </nav>
            <div className="topbar-actions">
              <button className="topbar-search-form" aria-label="Search anime" onClick={() => setIsSearchOpen(true)}><SearchIcon size={18} /><span>Search anime...</span><kbd>⌘K</kbd></button>
              {user ? <FocusableLink to={ownProfilePath}><User size={14} /><span>{user.username}</span></FocusableLink> : authLoading ? <div>Checking session...</div> : <FocusableButton onClick={() => { setAuthTab('login'); setShowAuthModal(true); }}><User size={14} /><span>Sign In</span></FocusableButton>}
            </div>
          </header>
          ...
          <main className="content">
            <Routes>
              ...
              <Route path="/dmca" element={<StaticPages page="dmca" />} />
              <Route path="/request" element={<StaticPages page="request" />} />
          ...
          <nav className="bottom-nav">
            <NavLink to="/" end className={({ isActive }) => isActive ? 'bottom-nav-link active' : 'bottom-nav-link'}><HomeIcon size={20} /><span>Home</span></NavLink>
            <NavLink to="/anime" className={({ isActive }) => isActive ? 'bottom-nav-link active' : 'bottom-nav-link'}><span style={{ fontSize: '.8rem', fontWeight: 'bold' }}>アニメ</span><span>Anime</span></NavLink>
            <NavLink to="/dramas-movies" className={({ isActive }) => isActive ? 'bottom-nav-link active' : 'bottom-nav-link'}><TvIcon size={20} /><span>Dramas</span></NavLink>
            <NavLink to="/search" className={({ isActive }) => isActive ? 'bottom-nav-link active' : 'bottom-nav-link'}><SearchIcon size={20} /><span>Search</span></NavLink>
          </nav>
