...
export default function SetNewPassword() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      // Keep AnimeVault's legacy credential table synchronized for existing accounts.
      // Neon Auth remains the source of truth for the reset itself.
  ...
  return (
    <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24, background: 'var(--bg-primary, #05050a)' }}>
      <form onSubmit={submit} style={{ width: '100%', maxWidth: 440, padding: 32, borderRadius: 20, background: 'rgba(15,15,25,.85)', border: '1px solid rgba(255,26,117,.15)' }}>
        ...
        <input type="password" placeholder="New password" value={password} onChange={(e) => setPassword(e.target.value)} minLength={6} required style={{ boxSizing: 'border-box', width: '100%', padding: 12, marginBottom: 12, borderRadius: 10, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', color: '#fff' }} />
        <input type="password" placeholder="Confirm new password" value={confirm} onChange={(e) => setConfirm(e.target.value)} minLength={6} required style={{ boxSizing: 'border-box', width: '100%', padding: 12, marginBottom: 16, borderRadius: 10, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', color: '#fff' }} />
