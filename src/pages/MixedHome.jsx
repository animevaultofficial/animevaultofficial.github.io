...
export default function MixedHome({ mobile = false }) {
  ...
  const mobileDetailsPath = (type, item) => {
    const itemId = item?.tmdbId || item?.id;
    const itemTitle = item?.title || item?.name || '';
    if (itemId == null) return '/';
    return `/drama/${encodeURIComponent(String(itemId))}?type=${type === 'movie' ? 'movie' : 'tv'}${itemTitle ? `&title=${encodeURIComponent(itemTitle)}` : ''}`;
  };
  const openDetails = (type, item) => mobile ? navigate(mobileDetailsPath(type, item)) : navigate(`/watch/${type === 'movie' ? 'movie' : 'tv'}/${item?.tmdbId || item?.id}`);
  const Card = ({ item, type }) => mobile
    ? <button type="button" onClick={() => openDetails(type, item)} className="anime-card-v2" style={{ textDecoration: 'none', textAlign: 'left', border: 0, padding: 0, background: 'none', color: 'inherit', cursor: 'pointer' }}><div className="card-media"><img src={item.poster} alt={item.title || item.name} loading="lazy" decoding="async" /><div className="card-overlay"><div className="play-icon-wrapper"><Play fill="white" size={24} /></div></div></div><div className="card-info"><h3 className="card-title">{item.title || item.name}</h3><div className="card-meta"><span>{type === 'movie' ? 'MOVIE' : 'TV SHOW'}</span>{item.year && <><span className="dot">•</span><span>{item.year}</span></>}</div></div></button>
    : <Link to={`/watch/${type === 'movie' ? 'movie' : 'tv'}/${item.tmdbId || item.id}`} className="anime-card-v2" style={{ textDecoration: 'none' }}><div className="card-media"><img src={item.poster} alt={item.title || item.name} loading="lazy" decoding="async" /><div className="card-overlay"><div className="play-icon-wrapper"><Play fill="white" size={24} /></div></div></div><div className="card-info"><h3 className="card-title">{item.title || item.name}</h3><div className="card-meta"><span>{type === 'movie' ? 'MOVIE' : 'TV SHOW'}</span>{item.year && <><span className="dot">•</span><span>{item.year}</span></>}</div></div></Link>;

  return <section className="home-v2">
    <div className="hero-v2 hero-carousel-v2" style={{ position: 'relative' }}>
      {slide && <div className="carousel-slide active" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}><div className="hero-img-wrapper"><img src={image} alt={title} loading="eager" fetchPriority="high" decoding="async" /><div className="hero-overlay-v2" /></div><div className="hero-content-v2"><div className="hero-info-v2"><span className="hero-rank"><Sparkles size={14} /> Featured on AnimeVault</span><h1 className="hero-title-v2">{title}</h1><div className="hero-meta-v2"><span>{kind === 'movie' ? <Film size={16} /> : <Tv size={16} />}{kind === 'movie' ? 'Movie' : 'TV Show'}</span>{slide.year && <span><Calendar size={16} /> {slide.year}</span>}{slide.rating && <span><Star size={16} /> {slide.rating}</span>}</div><p className="hero-desc-v2">{slide.overview || slide.description || 'Watch movies and TV shows on AnimeVault.'}</p><div className="hero-btns-v2"><button className="btn-play-v2" onClick={() => openDetails(kind, slide)}><Play size={20} fill="black" /> Watch Now</button><button className="btn-info-v2" onClick={() => navigate('/dramas-movies')}><Info size={20} /> Browse</button></div></div></div></div>}
