...
const normalizeId = value => {
  const raw = String(value ?? '').trim().replace(/^tmdb-/i, '');
...
export default function DramaDetailPage({ params = {}, goBack, navigate }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        // A route can occasionally contain a stale/provider ID. Never show a
        // misleading "missing ID" error when we have a title that can recover it.
        let actualId = id;
        let data = null;

        if (id) {
          data = mediaType === 'movie' ? await fetchMovieDetails(id) : await fetchTVDetails(id);
        }

        if (!data && initialTitle) {
          const match = await findMediaByTitle(initialTitle, mediaType);
          if (match?.id != null) {
            actualId = normalizeId(match.id);
            if (actualId) {
        ...
        if (cancelled) return;
        if (!data || !actualId) {
          throw new Error(initialTitle
            ? `Could not find this ${mediaType === 'movie' ? 'movie' : 'TV show'} in TMDB.`
            : `Missing ${mediaType === 'movie' ? 'movie' : 'TV show'} ID.`);
        }
        ...
        if (mediaType !== 'movie' && Array.isArray(data.seasons) && data.seasons.length) {
          const firstSeason = data.seasons.find(s => Number(s?.season_number) > 0)?.season_number || 1;
  ...
  useEffect(() => {
    ...
    if (mediaType !== 'movie' && details?.seasons && resolvedId) {
      ...
      fetchTVSeasonDetails(resolvedId, selectedSeason)
        .then(d => { if (!cancelled) setEpisodes(Array.isArray(d?.episodes) ? d.episodes : []); })
        .catch(() => {});
    }
    return () => { cancelled = true; };
  ...
  const loadSource = async (season = selectedSeason, episode = selectedEpisode, provider = selectedProvider) => {
    if (!resolvedId) {
      setSourceError('This title has no valid TMDB ID, so playback cannot be prepared.');
      return;
    }
    ...
    try {
      ...
      else if (routeSourceUrl) setMediaSource(routeSourceUrl);
      else setSourceError(`No direct media source is available from ${provider}.`);
    } catch (err) {
      setMediaSource(routeSourceUrl || '');
      setSourceError(err?.message || `Unable to load ${provider}.`);
    } finally {
      setSourceLoading(false);
    }
  };

  const selectProvider = async provider => { setSelectedProvider(provider); if (showPlayer) await loadSource(selectedSeason, selectedEpisode, provider); };
  ...
  const requireWatch = async () => { if (!user) { setAuthTab('login'); navigate?.('profile'); return; } setShowPlayer(true); await loadSource(); };
  const changeEpisode = async episode => { setSelectedEpisode(episode); if (showPlayer) await loadSource(selectedSeason, episode); };
  const goNextEp = () => { const idx = episodes.findIndex(e => e.episode_number === selectedEpisode); if (idx >= 0 && idx < episodes.length - 1) changeEpisode(episodes[idx + 1].episode_number); };
  const goPrevEp = () => { const idx = episodes.findIndex(e => e.episode_number === selectedEpisode); if (idx > 0) changeEpisode(episodes[idx - 1].episode_number); };
