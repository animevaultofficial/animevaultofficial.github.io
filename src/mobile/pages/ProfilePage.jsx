...
import { Award, BarChart3, Bell, Camera, ChevronRight, Clock3, Download, Edit3, Heart, History, Image as ImageIcon, LogOut, Save, Settings, Sparkles, Tv, UserRound, X } from 'lucide-react';
...
export default function ProfilePage({ navigate }) {
  ...
  const [draftAvatar, setDraftAvatar] = useState(FALLBACK);
  ...
  useEffect(() => { if (!displayUser) return; const a = displayUser.avatar || FALLBACK; const b = user?.banner || ''; setAvatar(a); setBanner(b); setDraftAvatar(a); setDraftBanner(b); }, [displayUser, user?.banner]);
  useEffect(() => { if (user?.id) dbGetUserStats(user.id).then(x => setStats(x || {})).catch(() => {}); }, [user?.id]);

  if (!user) return <AuthPrompt />;

  const favorites = normalize(likes?.length ? likes : (getFavorites()?.animes || []));
  const watched = normalize(history || []);
  ...
  return <div className="av-profile-page">
    <section className="av-profile-hero">
      <div className="av-profile-banner" style={banner ? { backgroundImage: `url(${banner})` } : undefined}>
        <div className="av-profile-banner-glow" />
        {editing && <label className="av-profile-image-edit banner"><ImageIcon size={15} /> Banner<input type="file" accept="image/*" onChange={e => chooseImage(e, setDraftBanner)} /></label>}
      </div>
      <div className="av-profile-identity">
        <div className="av-profile-avatar-wrap"><img src={editing ? draftAvatar : avatar} alt="" className="av-profile-avatar" />{editing && <label className="av-profile-image-edit avatar"><Camera size={15} /><input type="file" accept="image/*" onChange={e => chooseImage(e, setDraftAvatar)} /></label>}</div>
        <div className="av-profile-name"><div><h1>{displayUser.username || 'AnimeVault User'}</h1>{(displayUser.is_verified || displayUser.is_admin) && <span className="av-profile-badge">✓ Verified</span>}</div><p>{displayUser.email}</p></div>
