    ...
    <section style={{ minHeight: 'calc(100vh - 64px)', display: 'grid', placeItems: 'center', padding: '28px 18px' }}>
      <div style={{ width: '100%', maxWidth: 430, padding: 24, borderRadius: 22, background: 'rgba(15,15,25,.92)', border: '1px solid rgba(255,26,117,.18)', boxShadow: '0 18px 60px rgba(0,0,0,.45)' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <p style={{ margin: '9px 0 0', color: '#929099', fontSize: 13, lineHeight: 1.55 }}>Enter the email linked to your AnimeVault account and we'll send you a secure reset link.</p>
        </div>
        ...
        {status !== 'sent' ? <form onSubmit={submit}>
          ...
          <button type="submit" disabled={status === 'loading'} style={{ width: '100%', marginTop: 13, padding: '13px 14px', border: 0, borderRadius: 12, background: '#ff1a75', color: '#05050a', fontWeight: 900, fontSize: 14, opacity: status === 'loading' ? .7 : 1 }}>{status === 'loading' ? <><Loader2 size={16} style={{ verticalAlign: 'middle', marginRight: 7 }} /> Sending...</> : 'Send Reset Link'}</button>
        </form> : <button type="button" onClick={() => { setStatus('idle'); setMessage(''); }} style={{ width: '100%', padding: 13, borderRadius: 12, border: '1px solid rgba(255,255,255,.09)', background: 'rgba(255,255,255,.04)', color: '#fff', fontWeight: 800 }}>Send Again</button>}
      </div>
    </section>
  );
}
