...
export default function AndroidVideoPlayer({ sourceUrl, poster, title, onNextEpisode, onPrevEpisode }) {
  ...
  useEffect(() => {
    ...
    const cleanupHls = () => { hlsRef.current?.destroy(); hlsRef.current = null; };
    const fail = () => { if (!disposed) { setLoading(false); setError('This stream could not be played.'); } };

    if (isHls && Hls.isSupported()) {
      const hls = new Hls({ enableWorker: true, lowLatencyMode: true, backBufferLength: 30, maxBufferLength: 30, manifestLoadingMaxRetry: 2, levelLoadingMaxRetry: 2, fragLoadingMaxRetry: 2 });
      hlsRef.current = hls;
    ...
    return () => { disposed = true; cleanupHls(); video.removeAttribute('src'); video.load(); video.removeEventListener('canplay', ready); video.removeEventListener('play', onPlay); video.removeEventListener('pause', onPause); video.removeEventListener('error', onError); };
  }, [sourceUrl, retryKey]);
  ...
  return <div ref={wrapRef} className="av-native-player-v2">
    ...
    <div className="av-native-player-overlay-v2">
      ...
      {loading && <div className="av-native-loading-v2"><RefreshCw className="av-spin" size={30} /><span>Loading stream…</span></div>}
      ...
      <div className="av-native-controls-v2">
        <button onClick={onPrevEpisode} aria-label="Previous episode"><SkipBack size={20}/></button>
        ...
        <button onClick={onNextEpisode} aria-label="Next episode"><SkipForward size={20}/></button>
